...
if ($PAGE->pagetype == "enrol-index" && $enrolconfig == "1") {
    ...
    if ($DB->record_exists('course', array('id' => $courseId))) {
        ...
        foreach ($sections as $sectionnum => $section) {
            if ($section->__get('uservisible')) {
                if ($sectioncount == 0) {
                    $contentdata['sections'][$sectioncount]['sectionactive'] = true;    
                }
                $contentdata['sections'][$sectioncount]['index'] = $sectioncount;
                $contentdata['sections'][$sectioncount]['name'] = get_section_name($COURSE->id, $sectionnum);
                $sectioncount += 1;
            }
        }
    ...
    $teachers = get_enrolled_users($courseContext, 'mod/quiz:deleteattempts', 0,'u.id');
    $managers = get_enrolled_users($courseContext, 'moodle/category:manage', 0,'u.id');
    
    // Some teachers can be managers.3
    $teachers = array_diff(array_keys($teachers), array_keys($managers));
    ...
    if (!empty($teachers)) {
        ...
        foreach ($teachers as $key => $teacher) {
            ...
            if ($firstteacher) {
                ...
                $instructors['data'] = \theme_remui\usercontroller::wdmGetUserDetails($teacher);
                ...
                $instructors['instructorid'] = $teacher;
                $templatecontext['instructors']['list'][] = $instructors;
